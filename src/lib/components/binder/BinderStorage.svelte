...
<div class="bg-gray-800 rounded-lg p-4 flex flex-col h-full">
	...
	{#if $cards.length > 0}
		...
		{#if showFilters}
			<div class="mb-2 p-2 rounded text-sm" transition:slide={{ duration: 200 }}>
				<div class="flex justify-between items-center mb-1">
					<span class="text-gray-300 dark:text-gray-400">Sort by:</span>
	...
	<div class="overflow-y-auto flex-1 pr-1 -mr-1 min-h-0">
		{#if $cards.length === 0}
			<p class="text-gray-500 dark:text-gray-600 text-center py-4 text-sm">No cards in storage...</p>
		{:else if filteredCardCodes.length === 0}
			<p class="text-gray-500 dark:text-gray-600 text-center py-4 text-sm">No items match your search/filters.</p>
		{:else}
			<div class="grid grid-cols-2 gap-2">
				{#each filteredCardCodes as item (item)}
					{#if isCardCode(item)}
						...
						{#if fullCard}
							<div 
								class="relative aspect-[2/3] border-2 border-gray-700 dark:border-gray-600 rounded group transition-all duration-200 hover:border-gold-400 dark:hover:border-gold-400"
								draggable="true"
							...
							>
								...
								<div class="absolute bottom-0 left-0 right-0 bg-black bg-opacity-70 dark:bg-black dark:bg-opacity-80 text-[0.6rem] leading-tight text-white dark:text-gray-100 p-1 opacity-0 group-hover:opacity-100 transition-opacity text-center">
									<div class="font-semibold truncate">{fullCard.name}</div>
									<div class="truncate">#{setIndex}/{set?.printedTotal}</div>
									<div class="truncate text-gray-300 dark:text-gray-400">{fullCard.rarity}</div>
								</div>
						...
						{:else}
							<div class="relative aspect-[2/3] border-2 border-dashed border-red-700 dark:border-red-800 rounded flex items-center justify-center text-center p-1">
								<span class="text-red-400 dark:text-red-500 text-xs">Data missing for {item}</span>
					...
					{:else} 
						...
						>
							...
							<div class="absolute bottom-0 left-0 right-0 bg-black bg-opacity-70 dark:bg-black dark:bg-opacity-80 text-[0.6rem] leading-tight text-white dark:text-gray-100 p-1 opacity-0 group-hover:opacity-100 transition-opacity text-center">
								...
								<div class="truncate text-gray-300 dark:text-gray-400 break-all">{item}</div> 
							</div>
							<button class="absolute top-1 right-1 bg-red-500 rounded-full p-0.5 opacity-0 group-hover:opacity-100 transition-opacity" on:click={() => removeItem(item)} > <X size={14} /> </button>
						</div>
					{/if}
