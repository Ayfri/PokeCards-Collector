...
export const load: PageServerLoad = async ({ url, locals }) => {
	...
	if (requestedUsername) {
		...
		if (loggedInUserProfile) {
			...
			isPublic = loggedInUserProfile.is_public; // Included for consistency
			title = 'My Wishlist';
			...
			if (wishlistError) {
				console.error(`Error fetching own wishlist for ${targetUsername}:`, wishlistError);
			} else {
				const wishlistCardCodes = new Set(wishlistItems?.map(item => item.card_code) || []);
				wishlistCards = allCards.filter(card => wishlistCardCodes.has(card.cardCode));
			}
		} else {
			// Not logged in, trying to view own wishlist
			title = 'My Wishlist';
