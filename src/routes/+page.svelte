<script lang="ts">
	...
	// Calculer les raretés dans le dernier set
	$: raritiesInLatestSet = [...new Set(latestSetCards.map((card: FullCard) => card.rarity).filter((r) => r))];
	...
	<section class="flex flex-col md:flex-row gap-8 mb-12">
		<div class="flex-1">
			...
			<div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
				<a
					href="/cards-list"
					class="bg-gray-800 p-4 rounded-lg shadow-lg text-center hover:bg-gray-700 transition-colors border border-transparent hover:border-gold-400"
				>
					{#if !NO_IMAGES}
		...
		<div class="flex-1 flex justify-center items-center">
			{#if latestSet}
				<a
					...
					class="relative w-full max-w-md transform hover:scale-105 transition-transform duration-300"
				>
					{#if !NO_IMAGES}
	...
	{#if latestSet}
		<section class="mb-12">
			...
			<div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
				<a
					href="/cards-list?set={encodeURIComponent(latestSet.name)}"
					class="block bg-gray-800 p-6 rounded-lg shadow-lg hover:bg-gray-700 transition-colors border border-transparent hover:border-gold-400"
				>
