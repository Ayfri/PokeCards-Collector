...
export async function uploadJapaneseCards() {
	...
	// Get list of valid Pokemon IDs from the database
	const { data: validPokemons, error: pokemonsError } = await supabase
		.from('pokemons')
		.select('id');

	if (pokemonsError) {
		throw new Error(`Error fetching valid Pokemon IDs: ${pokemonsError.message}`);
	}

	const validPokemonIds = new Set(validPokemons.map(pokemon => pokemon.id));
	...
	const cardsTransformed = jpCardsData.map(card => ({
		...
		image: card.image || null,
		name: card.name,
		pokemon_id: (card.pokemonNumber && validPokemonIds.has(card.pokemonNumber)) ? card.pokemonNumber : null,
		rarity: card.rarity || null,
		set_name: (card.setName && validSetNames.has(card.setName)) ? card.setName : null,
		supertype: card.supertype || null,
		types: card.types || null
	...
	// Count invalid Pokemon IDs for reporting
	const invalidPokemons = jpCardsData.filter(card => card.pokemonNumber && !validPokemonIds.has(card.pokemonNumber));
	if (invalidPokemons.length > 0) {
		const uniqueInvalidPokemons = [...new Set(invalidPokemons.map(card => `${card.name} (${card.pokemonNumber})`))];
		console.log(`⚠️  Found ${invalidPokemons.length} Japanese cards with invalid Pokemon IDs: ${uniqueInvalidPokemons.slice(0, 5).join(', ')}${uniqueInvalidPokemons.length > 5 ? '...' : ''}`);
	}
